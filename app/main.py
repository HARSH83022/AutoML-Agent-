...
def fetch_queued_runs(limit=10):
    conn = sqlite3.connect(DB)
    cur = conn.execute(
        "SELECT run_id, state_json FROM runs WHERE status='queued' ORDER BY created_at ASC LIMIT ?",
        (limit,),
...
# Safe step wrapper  
# ==============================================================  
def safe_step(fn, *args, run_id=None, step_name="", **kwargs):
    last_exc = None
    for attempt in range(1, MAX_RETRIES + 2):
        try:
            ...
            last_exc = e
            agent_log(run_id, f"ERROR {step_name}: {e}", agent="orchestrator")
...
def orchestrate_run(run_id, payload):
    try:
        ...
        # ------------------------------------
        # Step 1: Problem Statement Agent
        # ------------------------------------
        ...
        # Process PS result
        if isinstance(ps_res, dict) and "options" in ps_res:
        ...
        update_run_state(run_id, "running", {"phase": "dataset_search", "ps": ps})

        # ------------------------------------
        # Step 2: Dataset Agent
        # ------------------------------------
        ds_res = safe_step(
            get_or_find_dataset,
            run_id, ps, payload.get("user", {}),
            run_id=run_id, step_name="data_agent"
        )

        dataset_path = ds_res if isinstance(ds_res, str) else ds_res.get("dataset_uri")
        if not dataset_path:
            raise RuntimeError("Dataset agent returned no dataset path")

        update_run_state(run_id, "running", {"phase": "preprocessing"})

        # ------------------------------------
        # Step 3: Preprocessing Agent
        # ------------------------------------
        prep_res = safe_step(
            preprocess_dataset,
            run_id, dataset_path, ps,
            run_id=run_id, step_name="prep_agent"
        )

        # ------------------------------------
        # Step 4: AutoML Agent
        # ------------------------------------
        update_run_state(run_id, "running", {"phase": "training"})
        train_res = safe_step(
            run_automl,
            run_id,
            prep_res["train_path"],
            ...
            prefs,
            run_id=run_id,
            step_name="automl_agent",
        )

        if "task_type" in train_res:
            ps["task_type"] = train_res["task_type"]

        # ------------------------------------
        # Step 5: Evaluation Agent
        # ------------------------------------
        update_run_state(run_id, "running", {"phase": "evaluation"})
        eval_res = safe_step(
            evaluate_model,
            run_id,
            prep_res["test_path"],
        ...
        # ------------------------------------
        artifacts = {
            "plan": save_artifact(run_id, "plan.json", json.dumps(ps, indent=2)),
            "dataset": dataset_path,
            "evaluation": save_artifact(run_id, "evaluation.json", json.dumps(eval_res, indent=2)),
            "model": train_res["model_path"]
        }

        update_run_state(run_id, "completed", {"artifacts": artifacts})
        agent_log(run_id, "Run completed", agent="orchestrator")

    except Exception as e:
        update_run_state(run_id, "failed", {"error": str(e)}, last_error=str(e))
        agent_log(run_id, f"FAILED: {e}", agent="orchestrator")
...
def bg_worker():
    ...
    while True:
        try:
            for run_id, state in fetch_queued_runs():
                curr = read_run(run_id)
                if curr["status"] == "queued":
                    update_run_state(run_id, "running", {"phase": "starting"})
                    payload = state.get("payload", {})
                    executor.submit(orchestrate_run, run_id, payload)
        except Exception as e:
            agent_log("system", f"Worker error: {e}", agent="orchestrator")
...
def status(run_id: str):
    ...
    log_path = f"{ARTIFACT_DIR}/{run_id}_log.txt"
    if os.path.exists(log_path):
        with open(log_path) as f:
            r["log_tail"] = "".join(f.readlines()[-200:])
    else:
        r["log_tail"] = ""

    r["artifacts"] = [
        f for f in os.listdir(ARTIFACT_DIR) if f.startswith(run_id)
    ]
    return r
...
def ps_interactive(payload: dict = Body(...)):
    run_id = payload.get("run_id") or str(uuid.uuid4())
    have_ps = bool(payload.get("have_ps"))
    ps_text = payload.get("problem_statement", "")
    ...
    try:
        if have_ps and ps_text.strip():
            parsed = safe_step(
                parse_problem_or_generate,
                run_id, ps_text, preferences,
                run_id=run_id, step_name="ps_agent"
            )
            return {"status": "ok", "mode": "parsed", "ps_parsed": parsed}
        ...
        if "options" in gen:
            return {"status": "ok", "mode": "generated_options", "ps_options": gen["options"]}

        return {"status": "ok", "mode": "generated_options", "ps_options": [gen]}
