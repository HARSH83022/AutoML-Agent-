...
def _ask_llm_plan(ps: dict, X_sample: np.ndarray, y_sample: np.ndarray):
    prompt = f"""
You are an ML system designer. Given this problem statement and a small preview of the target y values:
Problem: {ps.get('raw_text') or ps}
y_preview: {list(map(lambda x: str(x), list(y_sample)[:10]))}

Decide task_type (classification|regression), a short list of candidate models, and a recommended hpo time in minutes.
Return JSON only:
{{ "task_type": "classification", "candidate_models":["xgboost","lgbm"], "hpo_suggestion":{{"time_minutes":10}} }}
"""
    try:
        return llm_generate_json(prompt) or {}
    except Exception:
...
def run_automl(run_id: str, train_npz_path: str, ps: dict, preferences: dict):
    try:
        ...
        agent_log(run_id, f"[automl_agent] failed loading train file {train_npz_path}: {e}", agent="automl_agent")
        raise
    ...
    # Ask LLM for plan (using small sample)
    try:
        sample_idx = np.random.choice(len(y_train), size=min(50, max(1, len(y_train))), replace=False)
        X_sample = X_train[sample_idx] if hasattr(X_train, "__len__") else X_train[:min(50, len(y_train))]
        y_sample = y_train[sample_idx] if hasattr(y_train, "__len__") else y_train[:min(50, len(y_train))]
    except Exception:
        X_sample = X_train[:min(50, len(y_train))]
        y_sample = y_train[:min(50, len(y_train))]

    llm_plan = _ask_llm_plan(ps, X_sample, y_sample)
    ...
    automl = AutoML()
    settings = {"time_budget": max(30, time_budget), "metric": metric, "task": task_type, "estimator_list": est_list, "log_file_name": f"artifacts/{run_id}_flaml.log"}

    agent_log(run_id, f"[automl_agent] starting AutoML: task={task_type} metric={metric} time_budget={settings['time_budget']}s estimators={est_list}", agent="automl_agent")
    ...
    try:
        ...
        try:
            joblib.dump(automl, model_path)
            agent_log(run_id, "[automl_agent] pickled entire automl object as fallback", agent="automl_agent")
        except Exception as e:
            agent_log(run_id, f"[automl_agent] failed to save model: {e}", agent="automl_agent")
    ...
    agent_log(run_id, f"[automl_agent] finished. best_model={res.get('model_path')}", agent="automl_agent")
    return res
